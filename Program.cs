...
namespace Sorting
{
    class Program
    {
        ...
        public double[] Sort(double[] numbers) => numbers.OrderBy(n => n).ToArray();  //Сортировка
        ...
        public void Output()
        {
            try
            {
                string inputStrig = Input();                           //Получение вводимой строки
                ValidationAndConversion(inputStrig, out double[] unsortedNumbers);   //Проверка на корректность и получение ассива чисел
                double[] sortedNumbers = Sort(unsortedNumbers);      //Сортировка
                Console.WriteLine(string.Join(" ", sortedNumbers)); //Вывод сортированной строки
            }
            catch
            {
                Console.WriteLine("Не все введеные значения являються числам, попробуйте еще раз."); //Исключение в случае ошибки
...
>>>>>>> 26cd6acc6318e0521b1f48836dfbcdf636821fc1
